...
def create_scanner():
    ...
    if request.method == 'POST':
        try:
            ...
            # Features - New 8 Advanced Scans
            enable_subdomain_scan = bool(request.form.get('enable_subdomain_scan'))
            enable_email_security_scan = bool(request.form.get('enable_email_security_scan'))
            enable_waf_scan = bool(request.form.get('enable_waf_scan'))
            enable_technology_scan = bool(request.form.get('enable_technology_scan'))
            enable_api_security_scan = bool(request.form.get('enable_api_security_scan'))
            enable_cloud_scan = bool(request.form.get('enable_cloud_scan'))
            enable_compliance_scan = bool(request.form.get('enable_compliance_scan'))
            enable_performance_scan = bool(request.form.get('enable_performance_scan'))
            ...
            scanner = Scanner(
                ...
                # New 8 advanced scans
                enable_subdomain_scan=enable_subdomain_scan,
                enable_email_security_scan=enable_email_security_scan,
                enable_waf_scan=enable_waf_scan,
                enable_technology_scan=enable_technology_scan,
                enable_api_security_scan=enable_api_security_scan,
                enable_cloud_scan=enable_cloud_scan,
                enable_compliance_scan=enable_compliance_scan,
                enable_performance_scan=enable_performance_scan,
...
@client_bp.route('/client/scanners')
@login_required
def scanners():
...
@client_bp.route('/client/statistics')
@login_required
def statistics():
    """Client statistics page"""
    # Get statistics for the current user
    total_scans = Scan.query.filter_by(user_id=current_user.id).count()
    total_scanners = Scanner.query.filter_by(user_id=current_user.id).count()
    
    # Get scan statistics by status
    completed_scans = Scan.query.filter_by(user_id=current_user.id, status='completed').count()
    pending_scans = Scan.query.filter_by(user_id=current_user.id, status='pending').count()
    failed_scans = Scan.query.filter_by(user_id=current_user.id, status='failed').count()
    
    # Get vulnerability statistics
    scans_with_vulnerabilities = db.session.query(Scan).filter(
        Scan.user_id == current_user.id,
        Scan.vulnerabilities_found > 0
    ).all()
    
    total_vulnerabilities = sum(scan.vulnerabilities_found for scan in scans_with_vulnerabilities)
    
    # Calculate average risk score
    completed_scans_list = Scan.query.filter_by(
        user_id=current_user.id, 
        status='completed'
    ).all()
    
    avg_risk_score = 0
    if completed_scans_list:
        avg_risk_score = sum(scan.risk_score for scan in completed_scans_list) / len(completed_scans_list)
    
    return render_template('client/client-statistics.html',
                         user=current_user,
                         total_scans=total_scans,
...
@client_bp.route('/client/reports')
@login_required
def reports():
    """Client reports page"""
...
@client_bp.route('/client/reports/<scan_id>')
@login_required
def view_scan_report(scan_id):
    """View individual scan report"""
    # Find scan by scan_id
...
@client_bp.route('/client/settings')
@login_required
def settings():
