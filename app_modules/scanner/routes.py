...
from models import db, Scanner, Scan, ScannerCustomization, User, Lead
...
def perform_scan(scan_id):
    ...
    try:
        ...
        if scanner.enable_subdomain_scan:
            scan_types.append('subdomains')
        if scanner.enable_email_security_scan:
            scan_types.append('email_security')
        if scanner.enable_waf_scan:
            scan_types.append('waf')
        if scanner.enable_technology_scan:
            scan_types.append('technology')
        if scanner.enable_api_security_scan:
            scan_types.append('api_security')
        if scanner.enable_cloud_scan:
            scan_types.append('cloud_misconfig')
        if scanner.enable_compliance_scan:
            scan_types.append('compliance')
        if scanner.enable_performance_scan:
            scan_types.append('performance')
        
        # If no specific scans enabled, use all scans (recommended)
        if not scan_types:
...
def send_scan_email_report(scan):
    ...
    # Create lead record if contact email is provided
    if scan.contact_email:
        try:
            ...
            if not existing_lead:
                lead = Lead(
                    ...
                    scan_id=scan.id,
                    name=scan.contact_name or '',
                    email=scan.contact_email,
                    company=scan.contact_company or '',
                    phone=scan.contact_phone or '',
                    source='scanner',
                    notes=f'Generated from scan of {scan.domain}'
                )
                db.session.add(lead)
                db.session.commit()
                logger.info(f"Lead created for {scan.contact_email}")
            else:
                logger.info(f"Lead already exists for {scan.contact_email}")
        except Exception as e:
